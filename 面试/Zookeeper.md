...
存储数据的，所以Zookeeper中每个Znode的数据存储上限是1M。Znode主要分为4种类型，持久节点，临时节点，持久顺序节点和临时顺序节点，持久(PERSISTENT)节点是创建
...
(节点之间无法通信)时也需要对外提供服务。由于CAP无法同时满足，P必须满足，所以分布式集群需要满足AP或CP，Eureka实现的是AP，Zookeeper实现的CP，保证分布式数据一致性。
...
会获取一个全局唯一递增的提案编号N，Proposer将N发送给所有的Acceptor，Acceptor在本地保存prepare阶段响应过的最大提案编号MaxN和accept阶段的提交的提案编号AcceptN和对应的值AcceptV，
当Acceptor收到Proposer发送的N时会和本地的MaxN比较，若MaxN==null则是第一次响应，直接反馈成功，若MaxN>N则不回应或回复错误。若MaxN<N则更新本地的MaxN并且反馈Acceptor本地的AcceptN和AcceptV。
accept阶段是当Proposer收到半数以上的Acceptor的批准，那么Proposer会判断prepare阶段的返回，若没有返回AcceptN和AcceptV就会发送提案编号N和任一的值V发送。若prepare阶段返回AcceptN和AcceptV则
在返回中取最大的AcceptN对应的AcceptV和提案编号N一起返回。Acceptor收到提案编号和内容后和Acceptor的本地的MaxN比较，如果收到的N>=MaxN则accept提案，反馈给Proposer，不满足则返回错误或不返回。
当Proposer收到半数以上accept请求的反馈时会给所有的Acceptor发送提交请求，对于没有返回accept反馈的Acceptor，Proposer会发送提案编号和内容进行强制执行和提交。若Proposer在prepare和accept阶段
没有收到半数以上的Acceptor对请求的反馈时将对提案编号递增，然后重新进入prepare阶段。Paxos算法会出现死循环问题，当两个Proposer在prepare阶段先后向Acceptor发送提案N1和N2，Acceptor通过后反馈信息，
本地的MaxN=N2，当N1的Proposer进行accept阶段时，由于Acceptor的本地MaxN=N2>N1，所以N1的Proposer的accept阶段不成功，Proposer对N1进行自增变成N3重复prepare阶段，Acceptor的本地MaxN=N3，
然后N2的Proposer的accept阶段也不成功，自增后重复prepare阶段，导致Proposer持续自增陷入死循环。解决方法是选定只有一个proposer可以对Acceptor提出提案。
