...
就算不是严格按照索引顺序，引擎也会自动优化成匹配索引的顺序。至于为什么需要最左匹配才能命中索引是和InnoDB的B+Tree索引结构决定的，当建立联合索引时，索引值有是多个，但是
B+Tree只能按照一个key的顺序来构建，所以联合索引时B+Tree的非叶子节点存储的是最左边的索引key，叶子节点存储的是所有索引key和对应的主键id，因此B+Tree是对最左索引进行排序
构建的，在叶子节点中，对于相同的最左索引key，按照联合索引的顺序进行排序。即(a,b,c)联合索引，非叶子节点按照a排序，叶子节点中a相同的按照b排序，b相同的按照c排序。因此在叶子节点中
a是有序的，但是b，c是无序的。因此在联合索引中出现范围查询时，范围查询后面的字段无法使用索引，比如a>1 and b=2，a可以命中索引b不行，因为a查询的是一个范围，而b只是在a相同的情况下
才是有序，在a的范围中是无序的，所以无法命中索引。

![timewoo](https://timewoo.github.io/images/mysql4.jpg)
